...
│   ├── personas/               # Persona folders (per-provider personality)
...
├── slash_commands.py         # Slash command handling (/provider, /skill, /persona create multi-turn flow)
...
- **personas/<slug>/SOUL.md** — Primary persona format. `SOUL.md` contains YAML frontmatter (`Name`, `Description`, `Provider`) and body text. Loaded when `preferences.active_persona` is set.
- **SOUL.md**, **goals.md** — Legacy fallback personality and goals files from onboarding/manual edits.
...
- **Slash commands** (Phase 5b): `/help`, `/clear`, `/compact [keep]`, `/new`, `/rename <name>`, `/session <id|index|name>`, `/sessions`, `/skill <name>` (backend-handled), `/provider-settings`, `/provider list`, `/provider set <name>`, `/persona create` (backend-handled multi-turn flow; also accepts "create persona"); inline autocomplete when typing `/`
